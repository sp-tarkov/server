    ...
    protected filterEquipment(
        ...
        if (whitelist) {
            for (const equipmentSlotKey in baseBotNode.inventory.equipment) {
                ...
                baseBotNode.inventory.equipment[equipmentSlotKey] = Object.keys(botEquipment)
                    .filter((tpl) => whitelistEquipmentForSlot.includes(tpl))
        ...
        if (blacklist) {
            for (const equipmentSlotKey in baseBotNode.inventory.equipment) {
                ...
                baseBotNode.inventory.equipment[equipmentSlotKey] = Object.keys(botEquipment)
                    .filter((tpl) => !equipmentSlotBlacklist.includes(tpl))
                    .reduce((res, key) => ((res[key] = botEquipment[key]), res), {});
            }
        }
    }
    ...
    protected filterCartridges(
        ...
        if (whitelist) {
            for (const ammoCaliberKey in baseBotNode.inventory.Ammo) {
                ...
                baseBotNode.inventory.Ammo[ammoCaliberKey] = Object.keys(botAmmo)
                    .filter((tpl) => whitelist.cartridge[ammoCaliberKey].includes(tpl))
        ...
        if (blacklist) {
            for (const ammoCaliberKey in baseBotNode.inventory.Ammo) {
                ...
                baseBotNode.inventory.Ammo[ammoCaliberKey] = Object.keys(botAmmo)
                    .filter((tpl) => !cartridgeCaliberBlacklist.includes(tpl))
                    .reduce((res, key) => ((res[key] = botAmmo[key]), res), {});
            }
        }
    }
