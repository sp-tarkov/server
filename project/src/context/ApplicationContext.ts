...
export class ApplicationContext {
    ...
    public addValue(type: ContextVariableType, value: any): void {
        ...
        if (this.variables.has(type)) list = this.variables.get(type);
        else list = new LinkedList<ContextVariable>();

        if (list.getSize() >= ApplicationContext.holderMaxSize) list.removeFirst();
