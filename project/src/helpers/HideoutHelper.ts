...
export class HideoutHelper {
    ...
    public registerProduction(
        ...
        const modifiedProductionTime =
            recipe.productionTime - this.getCraftingSkillProductionTimeReduction(pmcData, recipe.productionTime);
        ...
        for (let i = 0; i < generatorArea.slots.length; i++) {
            if (generatorArea.slots[i].item) {
                ...
                if (resourceValue === 0) {
                    ...
                    resourceValue =
                        generatorArea.slots[i].item[0]._tpl === fuelItem ? 60 - fuelDrainRate : 100 - fuelDrainRate;
            ...
            for (let i = 0; i < waterFilterArea.slots.length; i++) {
                ...
                if (waterFilterArea.slots[i].item) {
                    ...
                    if (!resourceValue) {
                        ...
                        pointsConsumed =
                            (waterFilterArea.slots[i].item[0].upd.Resource.UnitsConsumed || 0) + filterDrainRate;
    ...
    protected updateBitcoinFarm(pmcData: IPmcData, btcFarmCGs: number, isGeneratorOn: boolean): Production {
        ...
        const bitcoinProdData = this.databaseServer
            .getTables()
            .hideout.production.find((p) => p._id === "5d5c205bd582a50d042a3c0e");
    ...
    protected getBTCSlots(pmcData: IPmcData): number {
        const bitcoinProduction = this.databaseServer
            .getTables()
            .hideout.production.find((p) => p._id === HideoutHelper.bitcoinFarm);
