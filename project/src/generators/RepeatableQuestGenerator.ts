...
export class RepeatableQuestGenerator {
    ...
    protected generateEliminationQuest(
        ...
        if (targetsConfig.data(targetKey).isBoss) {
            ...
            const thisBossSpawns = bossSpawns
                .map((x) => ({ Id: x.Id, BossSpawn: x.BossSpawn.filter((e) => e.BossName === targetKey) }))
    ...
    protected generateExplorationQuest(
        ...
        if (Math.random() < repeatableConfig.questConfig.Exploration.specificExits.probability) {
            ...
            const mapExits = (
                this.databaseServer.getTables().locations[locationKey.toLowerCase()].base as ILocationBase
            ).exits;
    ...
    protected generatePickupQuest(
        ...
        const equipmentCondition = counterCreatorCondition._props.counter.conditions.find(
            (x) => x._parent === "Equipment"
        );
        ...
        if (chosenRewardItems.length > 0) {
            for (let i = 0; i < rewardNumItems; i++) {
                ...
                if (roublesBudget > 0) {
                    ...
                    chosenRewardItems = chosenRewardItems.filter(
                        (x) => this.itemHelper.getStaticItemPrice(x._id) < roublesBudget
                    );
                    if (chosenRewardItems.length === 0) {
    ...
    protected chooseRewardItemsWithinBudget(
        ...
        let itemSelection = rewardableItems
            .filter(
                (x) =>
    ...
    protected generateRepeatableTemplate(type: string, traderId: string, side: string): IRepeatableQuest {
        ...
        */
        ...
        quest.successMessageText = quest.successMessageText
            ...
            .replace("{templateId}", quest.templateId);
        quest.failMessageText = quest.failMessageText
            .replace("{traderId}", traderId)
            .replace("{templateId}", quest.templateId);
        quest.startedMessageText = quest.startedMessageText
            .replace("{traderId}", traderId)
            .replace("{templateId}", quest.templateId);
        quest.changeQuestMessageText = quest.changeQuestMessageText
            .replace("{traderId}", traderId)
            .replace("{templateId}", quest.templateId);
        quest.acceptPlayerMessage = quest.acceptPlayerMessage
            .replace("{traderId}", traderId)
            .replace("{templateId}", quest.templateId);
        quest.declinePlayerMessage = quest.declinePlayerMessage
            .replace("{traderId}", traderId)
            .replace("{templateId}", quest.templateId);
        quest.completePlayerMessage = quest.completePlayerMessage
            .replace("{traderId}", traderId)
            .replace("{templateId}", quest.templateId);
